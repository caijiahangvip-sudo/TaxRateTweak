...
namespace TaxRateTweak.Systems
{
    public partial class FiveDensityDemandSystem : GameSystemBase
    {
        ...
        public bool Recalculate(ResidentialDemandSystem vanilla)
        {
            ...
            {
                m_Logged = true;
                Mod.log.Info($"Five-density demand 1.8.29 ready: low/row/medium/high/low-rent; properties {string.Join("/", Total)}; unlocked {string.Join("/", m_Unlocked)}; demand {Levels.Low}/{Levels.Row}/{Levels.Medium}/{Levels.High}/{Levels.LowRent}; no spawn culling.");
            }
            return true;
        }
